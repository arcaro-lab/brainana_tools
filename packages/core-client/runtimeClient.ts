...
/** Base path for a source's API namespace: `/api/sources/<id>`. Shared by the file + export clients. */
export function sourceBase(sourceId: string): string {
...
export class RuntimeClient {
  ...
  async apiJson<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    // Parse defensively: a non-2xx response may carry an HTML/plain-text body (proxy error,
    // SPA fallback), which must surface as `Request failed (status)`, not a raw SyntaxError.
   ...
   * no `?token=` query is appended: it would both corrupt NiiVue's file-extension detection
   * and leak the token into URLs/history (the server rejects `?token=` for that reason).
   */
